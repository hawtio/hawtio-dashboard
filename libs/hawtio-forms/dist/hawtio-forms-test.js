...
(function (Forms1Tests) {
    ...
    _module.config(['$routeProvider', 'HawtioNavBuilderProvider', function ($routeProvider, builder) {
        tab = builder.create().id(pluginName).rank(1).title(function () {
            return "Forms";
        }).href(function () {
            return "/forms";
        }).subPath("Simple Form", "simple_form", builder.join(tp, "test.html"), 1).subPath("Form Table", "form_table", builder.join(tp, "testTable.html"), 2).subPath("Wizard", "form_wizard", builder.join(tp, "wizard.html"), 3).build();
...
(function (Forms2Tests) {
    ...
    var tab2 = null;
    _module.config(['$routeProvider', 'HawtioNavBuilderProvider', function ($routeProvider, builder) {
        welcomeTab = builder.create().id('welcome').rank(10).title(function () {
            return "Documentation";
        }).href(function () {
            return "/docs";
        }).subPath("Welcome", "welcome", builder.join(tp, "welcome.html"), 1).build();
        tab2 = builder.create().id(pluginName + '-form2').rank(2).title(function () {
            return "Forms2";
        }).href(function () {
            return "/forms2";
        }).subPath("Simple Form", "simple_form", builder.join(tp, "simpleForm2.html"), 9).subPath("Map", "map", builder.join(tp, "map.html"), 9).subPath("Tabbed Form", "tabbed_form", builder.join(tp, "tabbedForm2.html"), 8).subPath("Wizard Form", "wizard_form", builder.join(tp, "wizardForm2.html"), 7).subPath("Nested Form", "nested_form", builder.join(tp, "nestedForm2.html"), 6).subPath("Schema Test", "from_schema", builder.join(tp, "fromSchema.html"), 3).build();
    ...
    _module.controller("HawtioFormsTests.Forms2SchemaController", ["$scope", "$templateCache", "SchemaRegistry", function ($scope, $templateCache, schemas) {
        ...
        $scope.config.style = 0 /* STANDARD */;
        $scope.config.mode = 1 /* EDIT */;
        $scope.model = {};
